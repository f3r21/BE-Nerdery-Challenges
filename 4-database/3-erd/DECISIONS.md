...
| 9 | `stock_notifications` has no table | `REVIEW.md` B4, shape from `REVIEW.md:186` | **Accepted, with a changed key** | `c41729f` | WRITE THIS. Facts: Feature 8 is the only feature that sends this mail, and nothing else in the schema records that it went. `REVIEW.md:186` proposed `UNIQUE (user_id, product_id)` and named no primary key, which would have left this the only table in the schema without one. It is a composite `[pk]` instead, matching `product_likes`, the structurally identical join table. The key is permanent either way, so a restock that falls to 3 again never notifies the same user twice. Name that cost here before he finds it. |
| 10 | Cart lines not unique per variant | `REVIEW.md` B6 | Accepted | `c41729f` | WRITE THIS. Fact: `openapi.yaml:1144` promises add-to-cart is idempotent, and `UNIQUE (cart_id, product_variant_id)` is what makes that sentence true. |
| 11 | Variant rows not unique | `REVIEW.md` B6 | Accepted | `23f98b7` | WRITE THIS. Fact: `UNIQUE (product_id, size, color)`. Nothing today stops a second Medium/Black row for the same product, and two rows then disagree about `stock` and `price`. |
| 12 | `promo_codes` has no usage counter | `REVIEW.md` B8 | Accepted | `23f98b7` | WRITE THIS. Fact: `usage_limit` is checked against nothing. `times_used` is the row a redemption locks, so two concurrent redemptions cannot both pass the limit. |
| 13 | Non-FK required columns nullable | `REVIEW.md`, same class as row 5 | **Partially accepted** | `23f98b7` | WRITE THIS. Fact: `not null` on `users.email`, `users.password_hash` and `products.name`. `quantity` and `price` deliberately left, and the reason belongs in Still open below. |
| 14 | Table names too long | Mentor item 6 | Accepted | `35597a4` | WRITE THIS. Facts: `user_auth_data` to `users`, `user_role` to `roles`, and 6 `Ref:` lines followed. This was one of his six and it is the only one of the six that was still open going into today. |
...
- **"Stock reaches 3" is undefined at the variant grain.** `REVIEW.md` B4b. A product has many
  ...
  close it.
- **`quantity` and `price` are still nullable.** Same class as row 5 and row 13, and left out
  of row 13 on purpose. WRITE THIS: the reason. `quantity` on two tables and `price` on
  `product_variants` are the three columns involved.
